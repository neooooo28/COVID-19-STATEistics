...
jh_daily<-data_US %>% group_by(date,state) %>% summarise(confirmed=sum(confirmed), deaths=sum(deaths)) #clean out date column
...
##Data Dictionary: CovidTracking 
### https://covidtracking.com/api (refer to Schemas/StatesHistoricalData)
positive: # of positive test results to date
negative: # of negative test results to date
hospitalizedCurrently: currently hospitalized patients
hospitalizedCumulatively: includes patients who are hospitalized currently + discharged patients
totalTestResults: total # of tests conducted to date
deathIncrease:increase from day before
posIncrease: increase in positive cases from day before
negIncrease:increase in neg cases from day before
totalTestResultsIncrease:increase in test results (positive and negative) from day before
```{r}
...
#Merge John Hopkins data(# of cases, deaths) w/CovidTracking testing data. John Hopkins has way more dates. Use a right join to show dates where we are missing COVIDTracking testing data. Beware, there's a lot of missing data for jan/february so use at your discretion!
...
```{r}
#Read in your data!
hospital_data<-read.csv("https://raw.githubusercontent.com/neooooo28/STATS141_Grp4/master/US%20Hospitals/us_hospitals.csv?token=ANLHLOIAESFTZTR3CBQHTXK6V6K5I")
...
#Merge beds_per_state with population data and most recent # of cases, add in inICU...
